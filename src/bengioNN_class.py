...
class BengioNN(nn.Module):
    def __init__(self, vocab_size, context_size, embed_size, hidden_dim):
        ...
        self.fc2 = nn.Linear(hidden_dim, 1)  # Output 1 for binary classification

    def forward(self, x):
        embedded = self.embedding(x)  # Shape: [batch_size, context_size, embed_size]
        summed_embeddings = torch.sum(embedded, dim=1)  # Sum over context_size
        hidden = torch.relu(self.fc1(summed_embeddings))
...
if __name__ == "__main__":
    ...
    print(f"Test Accuracy (BengioNN): {test_accuracy:.4f}")
