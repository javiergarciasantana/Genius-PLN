    ...
    with torch.no_grad():
        for _ in range(max_length):
            ...
            if len(input_tokens) < context_size:
                padded_input = [pad_idx] * (context_size - len(input_tokens)) + input_tokens
            else:
...
if __name__ == "__main__":
    ...
    # Generate text
    start_text = ["La"]
    generated_text = generate_language_with_top_k_sampling(bengio_nn, start_text, max_length=50, k=5, vocab=vocab, device=device)
